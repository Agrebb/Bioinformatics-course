...
int main(){
    ...
    vector <vector<pair<int, int>>> dp(n+1, vector<pair<int, int>>(m+1, {-inf, /*-1*/0}));
    dp[0][0] = {0, /*-1*/ 1};
    for (int i = 0; i <= n; i++){
        for (int j = 0; j <= m; j++){
            if (j > 0) {
                ...
                if (dp[i][j].first < dp[i][j-1].first - 1){
                    dp[i][j] = dp[i][j-1];
                    dp[i][j].first--;
                }
                else if (dp[i][j].first == dp[i][j-1].first - 1){
            ...
            if (i > 0) {
                ...
                else if (dp[i][j].first == dp[i-1][j].first - 1){
            ...
            if (i > 0 && j > 0 && s[i-1] == t[j-1]) {
                ...
                if (dp[i][j].first < dp[i-1][j-1].first + 1){
                    dp[i][j] = dp[i-1][j-1];
                    dp[i][j].first++;
                }
                else if (dp[i][j].first == dp[i-1][j-1].first + 1){
